...
def getCharmanderLink(soup):
    pokemon = soup.find("div", class_ = "infocard-list infocard-list-pkmn-lg")
    cards = pokemon.find_all("span", class_ = "infocard-lg-img")
    card = cards[3]
...
def findLetters(sentences):
    ...
    # find all the words that match the regular expression in each sentence
        matches = re.findall(exp, sentence)
